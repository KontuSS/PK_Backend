...
export const userProfilePhoto = pgTable(
  "user_profile_photo",
  {
    id: serial().primaryKey().notNull(),
  ...
  (table) => [
    ...
    check(
      ...
      sql`type = ANY (ARRAY['image/jpeg'::text, 'image/jpg'::text, 'image/png'::text, 'image/webp'::text])`
    ),
  ]
);

export const intrests = pgTable(
  "intrests",
  {
    id: serial().primaryKey().notNull(),
    name: text().notNull(),
  ...
  (table) => [unique("intrests_name_key").on(table.name)]
);
...
export const friendRequests = pgTable(
  ...
  (table) => [
    ...
    unique("friend_requests_sender_id_receiver_id_key").on(
      table.senderId,
      table.receiverId
    ),
    check(
...
export const messages = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`message_type = ANY (ARRAY['text'::text, 'image/JPEG'::text, 'image/JPG'::text, 'image/PNG'::text, 'image/WEBP'::text, 'link'::text, 'voice'::text, 'file/TXT'::text, 'folder'::text, 'ZIP'::text])`
    ),
  ]
);
...
export const repositoryMetadata = pgTable(
  "repository_metadata",
  {
...
export const repoEntries = pgTable(
  "repo_entries",
  {
    id: serial().primaryKey().notNull(),
    name: text().notNull(),
...
export const repoEntriesData = pgTable(
  "repo_entries_data",
  {
  ...
  (table) => [
    ...
    check(
      ...
      sql`extension = ANY (ARRAY['txt'::text, 'py'::text, 'java'::text, 'cpp'::text, 'js'::text, 'html'::text, 'css'::text, 'json'::text, 'xml'::text, 'md'::text, 'kt'::text])`
    ),
    check(
      "repo_entries_data_check",
      sql`((is_directory = true) AND (extension IS NULL) AND (content IS NULL) AND (number_of_lines IS NULL)) OR (is_directory = false)`
    ),
    check("repo_entries_data_size_check", sql`size >= 0`),
  ]
);

export const tokenizedcodes = pgTable(
  "tokenizedcodes",
  {
    id: serial().primaryKey().notNull(),
...
export const interestCategories = pgView("interest_categories", {
  id: integer(),
  name: text(),
  description: text(),
}).as(sql`SELECT id, name, description FROM intrests ORDER BY name`);

export const userWithInterests = pgView("user_with_interests", {
  ...
  sql`SELECT u.id AS user_id, u.nick, u.first_name, u.last_name, ui.category_ids AS interest_ids, array_agg(i.name) FILTER (WHERE i.id IS NOT NULL) AS interest_names FROM users u LEFT JOIN user_interests ui ON u.id = ui.user_id LEFT JOIN LATERAL unnest(ui.category_ids) interest_id(interest_id) ON true LEFT JOIN intrests i ON interest_id.interest_id = i.id WHERE u.is_active = true GROUP BY u.id, u.nick, u.first_name, u.last_name, ui.category_ids`
);

export const pendingFriendRequests = pgView("pending_friend_requests", {
...
export const userFriendsView = pgView("user_friends_view", {
  ...
  sql`SELECT uf.user_id, u.nick AS user_nick, unnest(uf.friends) AS friend_id, f.nick AS friend_nick, f.first_name AS friend_first_name, f.last_name AS friend_last_name FROM user_friends uf JOIN users u ON uf.user_id = u.id JOIN users f ON f.id = ANY (uf.friends) WHERE u.is_active = true AND f.is_active = true`
);

export const userBlockedView = pgView("user_blocked_view", {
  userId: integer("user_id"),
  userNick: text("user_nick"),
  ...
  sql`SELECT ub.user_id, u.nick AS user_nick, unnest(ub.blocked_users) AS blocked_id, b.nick AS blocked_nick, b.first_name AS blocked_first_name, b.last_name AS blocked_last_name FROM user_blocked ub JOIN users u ON ub.user_id = u.id JOIN users b ON b.id = ANY (ub.blocked_users)`
);

export const userConversations = pgView("user_conversations", {
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const conversationMessages = pgView("conversation_messages", {
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  ...
  sql`SELECT m.id AS message_id, m.conversation_id, m.sender_id, s.nick AS sender_nick, m.receiver_id, r.nick AS receiver_nick, m.message_type, CASE WHEN m.message_type = 'text'::text THEN convert_from(m.content, 'UTF-8'::name) ELSE NULL::text END AS text_content, m.content AS binary_content, m.sent_at, m.is_read FROM messages m JOIN users s ON m.sender_id = s.id JOIN users r ON m.receiver_id = r.id ORDER BY m.sent_at`
);

export const highSimilarityCodes = pgView("high_similarity_codes", {
